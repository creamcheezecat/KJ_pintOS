...
페이지 테이블 엔트리의 주소를 반환합니다.
PML4E가 VADDR에 대한 페이지 테이블을 가지고 있지 않은 경우, 
동작은 CREATE에 따라 달라집니다.
CREATE가 true인 경우, 새로운 페이지 테이블이 생성되고 해당 
페이지 테이블 내의 포인터가 반환됩니다.
그렇지 않은 경우, null 포인터가 반환됩니다. */
uint64_t *
...
/* 가능한 모든 페이지 테이블 엔트리(pte)에 대해 FUNC를 적용합니다.
이는 커널을 포함한 모든 엔트리에 적용됩니다. */
bool
 ...
 * register. */
/* 페이지 디렉터리(PD)를 CPU의 페이지 디렉터리 기점 레지스터에 로드합니다. */
void
...
해당하는 물리 주소에 해당하는 커널 가상 주소를 반환하거나,
 ...
 * failed. */
/* 사용자 가상 페이지 UPAGE에서 커널 가상 주소 KPAGE로 식별되는 물리 프레임까지
 ...
 * in PML4. */
/* PML4 내 가상 페이지 VPAGE의 PTE에서 dirty 비트를 DIRTY로 설정합니다. */
void
...
/* PML4에 있는 가상 페이지 VPAGE의 PTE가 최근에 접근된 경우(true), 
즉 PTE가 설치된 시간과 마지막으로 지워진 시간 사이에 접근된 경우를 나타냅니다. 
PML4에 VPAGE에 대한 PTE가 없는 경우(false)에는 false를 반환합니다. */
bool
