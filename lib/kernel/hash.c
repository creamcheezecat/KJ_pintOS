...
/* 보조 데이터 AUX를 사용하여 해시 함수 HASH를 사용하여 해시 값을 계산하고,
비교 함수 LESS를 사용하여 해시 요소를 비교하기 위해 해시 테이블 H를 초기화합니다. */
bool
...
/* NEW를 해시 테이블 H에 삽입하고, 이미 동일한 요소가 테이블에 없는 경우
널 포인터를 반환합니다.
이미 동일한 요소가 테이블에 있는 경우, NEW를 삽입하지 않고 해당 요소를 반환합니다. */
struct hash_elem *
...
hash_replace(), hash_delete() 함수 중 하나를 사용하여 해시 테이블 H를 수정하는 경우,
ACTION이나 다른 곳에서 수행하더라도 정의되지 않은 동작이 발생합니다. */
void
...
/* Initializes I for iterating hash table H.
   ...
   iterators. */
/* 해시 테이블 H를 반복하는 데 사용할 I를 초기화합니다.
	...
	반복 도중 해시 테이블 H를 수정하는 경우, hash_clear(), hash_destroy(),
	hash_insert(), hash_replace(), hash_delete() 함수 중 하나를 사용하면
	모든 반복자가 무효화됩니다. */
void
...
/* FNV는 크기가 다른 데이터를 고정된 크기의 해시 코드로 매핑하는 데 사용*/
...
/* 해시 테이블 H의 버킷 수를 이상적인 값에 맞춰 변경합니다.
이 함수는 메모리 부족으로 인해 실패할 수 있지만,
이는 단지 해시 접근의 효율을 떨어뜨리는 결과를 초래할 뿐입니다.
그래도 계속 진행할 수 있습니다. */
static void
rehash (struct hash *h) {
	...
	/* 현재 사용할 버킷 수를 계산합니다.
	약간의 BEST_ELEMS_PER_BUCKET을 위해 한 개의 버킷을 원합니다.
